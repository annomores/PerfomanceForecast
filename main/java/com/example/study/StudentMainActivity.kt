...
class StudentMainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bottomNav.setOnItemSelectedListener { item ->
            when (item.itemId) {
                R.id.nav_dashboard -> {
                    // Здесь открывается главный экран (или обновляется)
                    Toast.makeText(this, "Главная", Toast.LENGTH_SHORT).show()
                    true
                }
                R.id.nav_profile -> {
                    // Здесь открывается профиль
        ...
        fun selectSubject(index: Int) {
            // Кнопки: выделяем выбранную
            btnOOP.background = getDrawable(if (index == 0) R.drawable.subject_btn_selected else R.drawable.subject_btn_unselected)
            btnMath.background = getDrawable(if (index == 1) R.drawable.subject_btn_selected else R.drawable.subject_btn_unselected)
            btnDiff.background = getDrawable(if (index == 2) R.drawable.subject_btn_selected else R.drawable.subject_btn_unselected)
